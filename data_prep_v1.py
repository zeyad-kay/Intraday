...
vpoc_close_dist = []
tpoc_close_dist = []
vpoc_price_high_dist = []
vpoc_price_low_dist = []
tpoc_price_high_dist = []
tpoc_price_low_dist = []
price_std = []
price_skew = []
price_kurt = []
...
for d in df.Day.unique():
    for idx in range(1, len(cutoff)):
        ...
        price_std.append(round(window.Price.std(),3))
        price_skew.append(round(window.Price.skew(),3))
        price_kurt.append(round(window.Price.kurt(),3))

        vol_price_std.append(round(groupings.Volume.std(),3))
        ...
        vpoc_tpoc_dist.append(vpoc_price - tpoc)
        vpoc_close_dist.append(vpoc_price - close_price)
        tpoc_close_dist.append(tpoc - close_price)
        vpoc_price_high_dist.append(high - vpoc_price)  #high - vpoc
        vpoc_price_low_dist.append(vpoc_price - low)    
        tpoc_price_high_dist.append(high -tpoc)    #high - tpoc
        tpoc_price_low_dist.append(tpoc - low)
...
df = pd.DataFrame({'period': period,'movement':movement, 'chg':chg, 'vpoc': vpoc, 'total_volume':total_volume, 
                   'vpoc_tpoc_dist': vpoc_tpoc_dist,
                   'vpoc_close_dist':vpoc_close_dist, 'tpoc_close_dist':tpoc_close_dist, 'vpoc_price_high_dist':vpoc_price_high_dist,
                   'vpoc_price_low_dist':vpoc_price_low_dist, 'tpoc_price_high_dist':tpoc_price_high_dist, 'tpoc_price_low_dist':tpoc_price_low_dist,
                   'price_std': price_std, 'price_skew': price_skew, 'price_kurt': price_kurt, 'vol_price_std':vol_price_std, 'vol_price_skew':vol_price_skew,
                   'vol_price_kurt':vol_price_kurt, 'trapped_longs':trapped_longs, 'trapped_shorts':trapped_shorts,
